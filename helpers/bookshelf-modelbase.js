...
module.exports = function(Bookshelf) {
	...
	modelExt.isNotOwnerScope = function(userId, attr = 'user_id') {
		if (!isInteger(userId) && !isArray(userId)) {
			userId = get(userId, 'state.user.id', null);
		}
		if (isInteger(userId)) {
			this.whereNot(attr, userId);
		} else if (isArray(userId)) {
			this.whereNotIn(attr, userId);
		}
		return this;
	};
	...
	modelExt.withUserSummary = function() {
		this.withSelect('user', ['first_name', 'last_name'], uq => {
			uq.withDeleted();
		});
		return this;
	};

	modelExt.withUser = function() {
		this.withSelect('user', ['first_name', 'last_name', 'role'], uq => {
			uq.withDeleted();
		});
		return this;
	};
